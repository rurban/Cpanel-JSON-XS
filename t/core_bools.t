# taken from JSON::PP, with sed 's,JSON::PP,Cpanel::JSON::XS,'
use strict;
use warnings;
use Cpanel::JSON::XS;
use Test::More;
BEGIN {
  ...
  plan skip_all => 'no support for core boolean options'
    unless Cpanel::JSON::XS->can('CORE_BOOL');
}
...
SKIP: {
    skip "core_bools option doesn't register as true for core bools without core boolean support", 1
        unless Cpanel::JSON::XS::CORE_BOOL;
...
SKIP: {
    skip "core boolean support needed to detect core booleans", 4
        unless Cpanel::JSON::XS::CORE_BOOL;
    BEGIN { Cpanel::JSON::XS::CORE_BOOL and warnings->unimport(qw(experimental::builtin)) }
    ok Cpanel::JSON::XS::is_bool($new_true), 'core true is a boolean';
    ok Cpanel::JSON::XS::is_bool($new_false), 'core false is a boolean';
...
SKIP: {
    skip "core boolean support needed to detect core booleans", 4
        unless Cpanel::JSON::XS::CORE_BOOL;
    ok Cpanel::JSON::XS::is_bool($should_true), 'decoded true is a boolean';
    ok Cpanel::JSON::XS::is_bool($should_false), 'decoded false is a boolean';

    ok Cpanel::JSON::XS::is_bool($should_true), 'decoded true is a core boolean';
    ok Cpanel::JSON::XS::is_bool($should_false), 'decoded false is a core boolean';
}
